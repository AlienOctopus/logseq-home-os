...
5. Run setup from the connected-H action when the Logseq plugin is installed. The bridge saves an official graph export and creates properties, ranges, and tag schemas; the native plugin completes class inheritance inside Logseq, avoiding HTTP identity-conversion bugs.
...
8. Create or repair the native `Home OS` dashboard. Its single top-level `Your home at a glance` block must carry `#HM Generated`; maintain only that generated subtree. Put the literal nested `#HomeCapture` example first, use the human headings `Your home`, `Rooms & areas`, `Appliances & equipment`, `Home systems`, `Manuals & records`, and `Inbox & review`, and list only real durable records returned by the native tags. Keep durable titles clean and human-readable; native `HM` tags—not title emoji—define record type. Remove legacy type markers from owned durable titles by UUID only after the helper's collision preflight and official export.
...
8. Resolve the source review into a new or existing durable page using object-first Item naming: `Item noun — Manufacturer Model · Space · Home nickname`. Example: `Microwave — KitchenAid KMHC319LSS00 · Kitchen · My home`. Do not put the home hierarchy first or use slash-based pseudo-hierarchy. Preserve an exact model such as `LFXS29766S/00` in `hm-model`, but use `LFXS29766S∕00` in the page title so Logseq does not interpret the slash as a namespace. Typed relationships and native tags—not title parsing—are authoritative. The helper normalizes legacy type emoji and migrates owned durable pages by UUID after an official export, so never create a second marked/unmarked record pair. If the owner has not supplied a home nickname, use `My home`, not the generic page title `Home`, which commonly collides with Logseq navigation and existing `#home` content.
...
- The connected-H toolbar action opens **Find in Home OS** and performs a deterministic inbox check. The Finder queries only canonical durable records through their five native `HM` tags, deduplicates by UUID, and opens the selected record directly. Captures, evidence, citations, and incidental mentions never appear. `Home OS: Find a record` opens the same Finder from the command palette. Its four nodes echo Logseq's graph model while the H identifies Home OS without competing with Logseq's native Home icon. On an uninitialized graph it becomes the guided setup surface.
