...
8. Resolve the source review into a new or existing durable page using `Home nickname · Space · Item` naming. Keep that durable title explicit for uniqueness; dashboard and record-page links shorten it for scanning. If the owner has not supplied a home nickname, use `My home`, not the generic page title `Home`, which commonly collides with Logseq navigation and existing `#home` content.
