...
// scripts/schema-transit-source.mjs
var import_transit_js = __toESM(require_transit(), 1);
var keyword = (value) => import_transit_js.default.keyword(String(value).replace(/^:/, ""));
var uuid = (value) => import_transit_js.default.uuid(value);
function buildSchemaImport(properties, classes = []) {
  const propertyEntries = [];
  for (const property of properties) {
    const spec = [
      keyword("block/title"),
      property.title,
      keyword("build/keep-uuid?"),
      true,
      keyword("logseq.property/type"),
      keyword(property.type),
      keyword("db/cardinality"),
      keyword(`db.cardinality/${property.cardinality}`),
      keyword("logseq.property/ui-position"),
      keyword("properties")
    ];
    ...
    if (property.choices?.length) {
      spec.push(
        ...
        property.choices.map((choice) => import_transit_js.default.map([
          keyword("uuid"),
          uuid(choice.uuid),
          keyword("value"),
          choice.value
    ...
    propertyEntries.push(keyword(property.ident), import_transit_js.default.map(spec));
  }
  const classEntries = [];
  for (const classSpec of classes) {
    ...
    classEntries.push(keyword(classSpec.ident), import_transit_js.default.map(spec));
  }
  const root = [keyword("properties"), import_transit_js.default.map(propertyEntries)];
  if (classEntries.length) root.push(keyword("classes"), import_transit_js.default.map(classEntries));
  return import_transit_js.default.writer("json").write(import_transit_js.default.map(root));
}
function readSchemaImport(payload) {
  return import_transit_js.default.reader("json").read(payload);
}
export {
  buildSchemaImport,
  readSchemaImport
